...
contract VaultSYActions_RPC_tests is DSTest, ERC1155Holder, ERC721Holder {
    ...
    function _mintUnderlier(address to, uint256 amount) internal {
        hevm.store(address(underlier), keccak256(abi.encode(address(address(this)), uint256(0))), bytes32(uint256(1)));
    ...
    function test_decreaseCollateral_matured() public {
        ...
        hevm.roll(block.number + 2102400); // accrue block based interest
        hevm.warp(maturity); // mature bond
